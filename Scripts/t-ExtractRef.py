...
for file in os.listdir("./articlesOA"):
    if file.endswith(".xml"):
        ...
        xml=xml.read()
        ref_list=xml.split("ref-list")[1].split("/ref-list")[0]
        file=codecs.open("./articlesOA/Content/"+(str(file).split(".")[0])+"_ref.xml","w",encoding="utf-8")
        file.write(ref_list)
        file.close()
        nb_done+=1
        advancement=(nb_done/length)*100
        print ("Content extraction of articles..........",str(int(advancement))+"%")
print ("Ref extraction of articles..... DONE")
