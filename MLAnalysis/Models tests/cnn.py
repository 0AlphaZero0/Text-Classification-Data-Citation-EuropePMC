...
def tokenizer(doc):
	...
	return tokens
#
def calc_score(vectorizer,approach):
	fold=0
...
output_file=codecs.open(
	...
	mode='w',
	encoding='utf8')
output_file.write("f1-score\tPrecision\tRecall\tAccuracy\tLoss\tf1-scoreCV\tPrecisionCV\tRecallCV\tAccuracyCV\tLossCV\tToken\tNgram\tRaw\tLemma\tStem\tTfidf\tEmbedding\tTime\n")
for vectorizer in ["Tfidf","Embedding"]:
	print(vectorizer)
	if vectorizer=="Tfidf":
		for combination in combinations_tfidf:
			f1_score_list,precision_list,recall_list,accuracy_list=[],[],[],[]
			val_acc_list,val_loss_list=[],[]
			featuresList.pop(-1)
			if combination[1]=="Raw":
				featuresList.append(completeCitation)
			elif combination[1]=="Stem":
				featuresList.append("stem_citation")
			else:
				featuresList.append("lemma_citation")
			print("Approach : "+combination[1]+" + "+combination[2])
			X=data[featuresList]
			...
			for train_index,test_index in skf.split(X_to_train,y_to_train):# cross validation
				...
				vect_X_train.append(combination[0].fit_transform(X_train[featuresList[-1]].fillna('').values.reshape(-1)).todense())
				vect_X_test.append(combination[0].transform(X_test[featuresList[-1]].fillna('').values.reshape(-1)).todense())
				vect_X_train.extend((
