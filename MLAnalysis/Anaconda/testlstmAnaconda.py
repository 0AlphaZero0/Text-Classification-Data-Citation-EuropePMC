...
without_punctuation=[]
without_punctuation_digits=[]
for citation in data[completeCitation]:
	without_punctuation.append(re.sub(r'[^\w\s\.]','',citation))
	without_punctuation_digits.append(re.sub(r'[^a-zA-Z\s\.]','',citation))
data["without_punctuation"]=without_punctuation
data["without_punctuation_digits"]=without_punctuation_digits
###########################################################################################
output_file=codecs.open(result_output,'w',encoding='utf8')
output_file.write("f1-score\tPrecision\tRecall\tAccuracy\tCross-score("+str(k_cross_val)+")\tLoss\tTime\tApproach\tPunctuation-digits\n")
for dataCitation in [data[completeCitation],data["without_punctuation"],data["without_punctuation_digits"]]:
	vocab_size=500
	lemma_citation=[]
	stem_citation=[]
	for citation in dataCitation:
	...
	approaches=[dataCitation,data["lemma_citation"],data["stem_citation"]]	
	...
	for approach in approaches:
		...
		for train_index,test_index in skf.split(X,y):
			...
			seq_features=layers.LSTM(200,activation='tanh',go_backwards=True)(embedding)
			...
			model = layers.Dense(128,activation='relu')(seq_features)
		...
		output_file.write("\t")
		output_file.write(str(dataCitation.name))
		output_file.write("\n")
output_file.close()