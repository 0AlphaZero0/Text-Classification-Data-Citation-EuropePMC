...
# Files
dataset_filename="Dataset231.csv"
result_outfile="ResultSVM.csv"
# Parameters
k_cross_val=4
average="macro"
gamma="auto"
...
clfSVM=svm.SVC(C=C,max_iter=max_iter,class_weight=class_weight)
#
################################################    Functions     #################################################
#
def lemma_word(word):
...
for index_vect_list in range(len(vect_list)):
	...
	for train_index,test_index in skf.split(X_to_train,y_to_train):
		...
		try:
			...
			y_pred_class=clfSVM.predict(X_test_dtm)
		except TypeError:
			clfSVM.fit(X_train_dtm.toarray(),y_train)
			y_pred_class=clfSVM.predict(X_test_dtm.toarray())
	...
	# y_pred_class_val=clfSVM.predict(X_val_dtm)
	result=clfSVM.predict_proba(X_val_dtm)
	y_pred_class_val=[]
